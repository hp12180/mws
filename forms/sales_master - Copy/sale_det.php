...
<script type="text/javascript">
 ...
 $(document).ready(function() {
		...
		$( "#billdt" ).datepicker();
    	  var a='dd/mm/yy';
		  $( "#billdt" ).datepicker("option","dateFormat",a);
...
var sc1=document.getElementById("sch1").value;
var sc2=document.getElementById("sch2").value;
if(qty1>0 && qty2<0)
{
  var amnt=qty1*rt;
...
var charges_typ=document.getElementById("charges").value;
var add_less=document.getElementById("other").value;
//alert(charges_typ)
if(charges_typ == "Add")
{
var total_last1=parseInt(total_last)+parseInt(add_less);
}
else
{
var total_last1=parseInt(total_last)-parseInt(add_less);
...
//alert(total_last1);
document.getElementById("sale_total").value=total_last1;

var dis_last=document.getElementById("dis1").value;

var final_dis=total_last1*dis_last/100;
document.getElementById("amnt").value=final_dis;
var total2=total_last1-final_dis;
//alert(total2);
...
function get_next_data(a)
	{
		...
		//alert(name);
		$("#basic-modal-content").load("forms/brand_master/fetch_brands.php",{item_name:name},function(responseText){  
				//alert(responseText)
		});
	...
	{
		//alert(b)
		var add_less=c.split("-");
		var no=document.getElementById(add_less[0]).innerHTML;
		var name=document.getElementById(add_less[0]+"-"+add_less[0]).innerHTML;
		document.getElementById("add_less").value=no;
		document.getElementById("add_less_name").value=name;
		event.preventDefault();
	...
	function fetch_item_details()
	{
		...
		//alert(name);
	     	/* here "demo" is the id of the div element where u want to write response */
			$("#demo").load("forms/brand_master/shw_unit.php",{js_code:code},function(responseText){  
	    ...
	    // $("#broker").change(function(){
		 ...
		 {	
			...
			else
			{
	     	...
	     	/* here "demo" is the id of the div element where u want to write response */
			$("#demo").load("forms/sales_master/sale_up2.php",{query_type:query_type,billno:billno,billdt:billdt,day:day,cust_name:cust_name,mod:mod,broker:broker,brand_name1:brand_name1,brand_code1:brand_code1,mrp1:mrp1,sale_qty:sale_qty,sale_qty1:sale_qty1,sale_rt:sale_rt,sale_rt1:sale_rt1,sale_amnt:sale_amnt,tax_typ:tax_typ,sale_vat:sale_vat,tax_amnt:tax_amnt,sch1:sch1,sch2:sch2,sale_dis:sale_dis,dis_amnt:dis_amnt,hamali:hamali,postage:postage,fright:fright,charges:charges,add_less:add_less,other:other,dis1:dis1,amnt:amnt,sale_total:sale_total,recv:recv,balance:balance},function(responseText){  
				alert(responseText)
...
<body>	
<div id="basic_add_less" style="height:400px;width:400px;padding-left:300px">
<table border="1" cellpadding="2" style="color:#CCCCCC">
						<?php
                           require_once("../conectivity/cone.php");
							$query1="select gr_type,acc_name from acc_det_information where acc_code in(101,102,103,104,105,106,426,427,428,429)";
							$result1=mysql_query($query1,$con);
							//echo "<tr>";
							$x=0; $y="b";
							while($row1=mysql_fetch_assoc($result1))
							{
								echo "<tr><td id='$x$y' ondblclick='get_next(this.id)'>".$row1['gr_type']."</td>"."<td id='$x$y-$x$y' ondblclick='get_next(this.id)'>".$row1['acc_name']."</td></tr>";;
...
<div id="basic-modal-content_broker" style="height:400px;width:400px;padding-left:300px">
  <?php
           include("../forms/sales_master/broker_details.php");
...
<table  style="border-spacing:10px;border-collapse:separate;width:980px;padding:20px">
<td>Bill_no</td>
...
<td><input class='mws-textinput' type="text" name="sch1" id="sch1" value="0" onchange="sale_cal();"/></td>
<td>Scheme2</td>
<td><input class='mws-textinput' type="text" name="sch2" id="sch2" value="0" onchange="sale_cal();"/></td>
</tr>
<tr>
<td>dis_per %</td>
...
<td><select name="charges" id="charges" onchange="sale_cal();"><option value="Add">Add</option><option value="Less">Less</option></select></td>

<td><input class='mws-textinput' type="text" name="add_less" id="add_less" /></td>
<td><input class='mws-textinput' type="text" name="add_less_name" id="add_less_name" /></td>

<td>Other a/c</td>

<td><input class='mws-textinput' type="text" name="other" id="other" value="0" onchange="sale_cal();"/></td>
...
<td><input class='mws-textinput' type="text" name="sale_total" id="sale_total" value="0" readonly=""/></td>
