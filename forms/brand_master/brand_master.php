...
$(document).ready(function(){
	...
	$('#view_all_brands_dialog').click(function (e) {
		 ...
		 }else
		 {
			  ...
			  if($('#brand_code_name')=='')
			  {
			  	//alert('Entered in IF : '+$('#brand_code_name'))
			  }
			  else
			  {
			  	$('#demo').append("<div id='brand_code_name'></div>");
			  }
	...
	$( "#item_name" ).bind( "autocompletechange", function() {
					...
					else
					{
						...
						document.getElementById("pcs").value=data[4];
						document.getElementById("bal").value=data[5];
						
						document.getElementById("pcs_hid").value=data[4];
						document.getElementById("bal_hid").value=data[5];
						
						document.getElementById("o_amnt").value=data[1];
						
						document.getElementById("item_upper_unit").innerHTML=data[2];
						document.getElementById("item_lower_unit").innerHTML=data[3];
...
function pur_sal_cal()
{
   ...
   if(pur)
   {
   		var pur_low=pur/upper_lower;
		document.getElementById("pur1").value=pur_low.toFixed(2);
	}
	
	if(sal)
	{
   		var sale_low=sal/upper_lower;
		document.getElementById("sal1").value=sale_low.toFixed(2);
	}
...
function update_upper_lower()
{
	var upper=parseFloat(document.getElementById("pcs_hid").value);
	var lower=parseFloat(document.getElementById("bal_hid").value);
	
	var get_upper=parseFloat(document.getElementById("qty1").value);
	var get_lower=parseFloat(document.getElementById("qty2").value);
	
	var new_upper=get_upper+upper;
	var new_lower=get_lower+lower;
	
	document.getElementById("pcs").value=new_upper.toFixed(2);
	document.getElementById("bal").value=new_lower.toFixed(2);
  ...
  //alert("Code : "+code);
	...
	//				document.getElementById("balance").value=str1[30];
			...
			$("#demo").load("forms/brand_master/brand_master_up2.php",{query_type:query_type,item_name:item_name,op_bal_dr:op_bal_dr,op_bal_cr:op_bal_cr,op_bal_pcs:op_bal_pcs,op_bal:op_bal,brand_code:brand_code,brand_name:brand_name,op_amnt:op_amnt,inventory_control:inventory_control,upper:upper,lower:lower,qty1:qty1,qty2:qty2,ul:ul,pcs_qty:pcs_qty,mrp:mrp,sales_tax:sales_tax,type:type,hamali:hamali,tolai:tolai,pur:pur,sal:sal,pur1:pur1,sal1:sal1,grp_code:grp_code},function(responseText){  
				...
				if(responseText.indexOf('Succssesfully')!=-1)
				{
					document.getElementById("my_div").style.width="1000px";
					document.getElementById("my_sub_div").style.padding="0px";
					document.getElementById("ques_header").innerHTML="Brand Master";
...
<td>Op Bal <span id='item_upper_unit' style='font-weight:bold'></span></td><td><input type='text' name='pcs' id='pcs'  value='0' class='mws-textinput' style='width:130px'/><input type='hidden' id='pcs_hid'/></td>
<td>Op Bal <span id='item_lower_unit' style='font-weight:bold'></span></td><td><input type='text' name='bal' id='bal' value='0' class='mws-textinput' style='width:130px'/><input type='hidden' id='bal_hid'/></td>
</tr>
<tr>
<td>Brand Code</td>
...
<div style=''>
	...
	<td><input type='text' name='qty1' id='qty1'  value='0' onChange='cal_pcs();update_upper_lower();' class='mws-textinput'/></td>
	<td>
	
	<table><tr>
	<td style='padding-left: 50px;'><input type='text' name='ul' id='ul' onChange='ul_unit();' class='mws-textinput'/></td></tr><tr>
	
	<td style='padding-left: 50px;'><input type='text' name='pcs_qty' id='pcs_qty' value='0' onChange='cal_pcs();pur_sal_cal()' class='mws-textinput'/></td></tr></table></td></tr>
	
	<tr>
	<td>Lower&nbsp;&nbsp;&nbsp;<input type='text' name='lwr' id='lwr' onChange='ul_unit();' class='mws-textinput'/></td>
	<td><input type='text' name='qty2' id='qty2' onChange='update_upper_lower();'  value='0' class='mws-textinput'/></td>
...
<td>Purchase&nbsp;&nbsp;&nbsp; <input type='text' name='pur1' id='pur1'  value='0' class='mws-textinput' />
    &nbsp;&nbsp;&nbsp;sale &nbsp;&nbsp;&nbsp;<input type='text' name='sal1' id='sal1' value='0'  class='mws-textinput' /></td>
</tr>
</table>
